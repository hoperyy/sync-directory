...
const copyFileOrFolder = ({ srcPath, srcDir, targetDir, exclude, forceSync, type }) => {
    ...
    try {
        if (!isDirectory && type === 'hardlink') { // only file
    ...
    chokidarWatcher.on('ready', () => {
        ...
        chokidarWatcher.on('add', async srcPath => {
            await actorProcessor({
                ...
                nodeType: 'file',
                async actor() {
                    await copyFileOrFolder({ srcPath, srcDir, targetDir, exclude, forceSync, type });
                }
            });
        });
