...
const readdirSync = (dir, deep, filter) => {
...
const readdirAsync = async (dir, deep, filter) => {
...
const syncProcessor = (srcDir, targetDir, { type, exclude, forceSync, deep, afterSync, deleteOrphaned, staySymlink, include }) => {
    const srcFiles = readdirSync(srcDir, deep, filter(srcDir, exclude, forceSync));

    const targetFiles = readdirSync(targetDir, deep, filter(targetDir, exclude, forceSync));
...
const asyncProcessor = async (srcDir, targetDir, { type, exclude, forceSync, deep, afterSync, deleteOrphaned, staySymlink, include }) => {
    const srcFiles = await readdirAsync(srcDir, deep, filter(srcDir, exclude, forceSync));

    const targetFiles = await readdirAsync(targetDir, deep, filter(targetDir, exclude, forceSync));
