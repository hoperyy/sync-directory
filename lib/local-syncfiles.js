...
const syncProcessor = (srcDir, targetDir, { type, exclude, forceSync, skipChildren, afterSync, deleteOrphaned, staySymlink, include }) => {
    const srcFiles = readdirSync(srcDir, skipChildren, filter(srcDir, exclude, forceSync));

    const targetFiles = readdirSync(targetDir, skipChildren, filter(targetDir, exclude, forceSync));
...
const asyncProcessor = async (srcDir, targetDir, { type, exclude, forceSync, skipChildren, afterSync, deleteOrphaned, staySymlink, include }) => {
    const srcFiles = await readdirAsync(srcDir, skipChildren, filter(srcDir, exclude, forceSync));

    const targetFiles = await readdirAsync(targetDir, skipChildren, filter(targetDir, exclude, forceSync));
