...
const copyFileOrFolder = ({ srcPath, srcDir, targetDir, exclude, forceSync, type, stayHardlink }) => {
    ...
    // ignored
    if (!forceSync(relativePath) && exclude(relativePath)) {
        return;
    }
...
const processor = (srcDir, targetDir, { type, stayHardlink, exclude, skipChildren, forceSync, afterSync, onError, chokidarWatchOptions }) => {
    ...
    chokidarWatcher.on('ready', () => {
        const actorProcessor = async ({ srcPath, eventType, nodeType, actor }) => {
            ...
            if (matchUtil.test(srcPath, ignoredSymlinkDirs)) {
                return;
            }

            if (skipChildren) {
                let rel = srcPath.replace(srcDir, '');
                ...
                // is children
                if (rel.split(path.sep).length > 1) {
                    return;
                }
            }
