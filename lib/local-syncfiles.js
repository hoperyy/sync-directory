...
const syncFiles = function (srcDirs, targetDirs, { type, exclude, forceSync, afterSync, deleteOrphaned, supportSymlink, filter, onError }) {
    try {
         ...
         */
        const srcFiles = srcDirs.map((srcDir, index) => readdirSync(srcDir, stats => {
            ...
            const targetPath = path.join(targetDirs[index], relPath);
            ...
            return true;
        }).map(file => [srcDir, targetDirs[index], file])).reduce(flatten);

        // array to json
        const wsJson = {};
        targetDirs.forEach(targetDir => readdirSync(targetDir).forEach(filePath => {
